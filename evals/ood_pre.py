...
from models.transform_layers import DiffusionCustomTrans
...
def eval_ood_detection(P, model, id_loader, ood_loaders, ood_scores, train_loader=None, simclr_aug=None):
    ...
    assert len(ood_scores) == 1, ood_scores  # assume single ood_score for simplicity
...
def _get_features(P, model, loader, interp=False, imagenet=False, simclr_aug=None,
    ...
    for i, (x, _) in enumerate(loader):
        ...
        for seed in range(sample_num):
            ...
            if P.K_shift > 1:
                if isinstance(P.shift_trans, DiffusionCustomTrans):
                    x_t = torch.cat([P.shift_trans(
                        x, k, diffusion_offset=P.diffusion_offset, diffusion_upper_offset=P.diffusion_upper_offset,
                        random=False
                    ) for k in range(P.K_shift)])
                else:
                    x_t = torch.cat([P.shift_trans(hflip(x), k) for k in range(P.K_shift)])
                # x_t = torch.cat([P.shift_trans(hflip(x), k, diffusion=False, diffusion_offset=0) for k in range(P.K_shift)])
            else:
